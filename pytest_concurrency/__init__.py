...
PLUGIN_IS_ENABLED: t.Final = (
    list(filter(lambda x: WORKERS_ARGUMENT in x, sys.argv))
    or envvars.PYTEST_CONCURRENCY_WORKERS in os.environ
)
    ...
    def pytest_runtestloop(session: Session) -> bool:
        ...
        if session.testsfailed and not continue_on_collection_errors:
            ...
            raise session.Interrupted(
                f"{session.testsfailed} error{error_suffix} during collection"
            )

        if session.config.option.collectonly:
            return True

        workers_count = get_workers_count(session.config, WORKERS_ARGUMENT)
        ...
        for test_suite in test_suites:
            ...
            worker = Thread(target=run_test_suite, args=(session, test_suite))
